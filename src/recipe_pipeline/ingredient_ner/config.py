...
def _stage_from_pipeline(
    cfg: Dict[str, Any],
    stage_key: str,
    sub_key: str | None = None,
) -> Dict[str, Any]:
    """
    Extract a stage-specific section from the consolidated pipeline config.

    Falls back to returning cfg unchanged if the expected stage is not present.
    """
    pipeline_cfg = cfg.get("pipeline")
    if not isinstance(pipeline_cfg, dict):
        return cfg

    stage_cfg = pipeline_cfg.get(stage_key)
    if not isinstance(stage_cfg, dict):
        return cfg

    if sub_key:
        stage_cfg = stage_cfg.get(sub_key, {})
        if not isinstance(stage_cfg, dict):
            raise ValueError(
                f"Stage '{stage_key}' in pipeline config does not contain '{sub_key}'."
            )
...
def _build_data_ns(cfg: Dict[str, Any]) -> SimpleNamespace:
    ...
    if not train_path_str:
        # Default to parquet version of the canonical raw dataset
        train_path_str = "../data/raw/wilmerarltstrmberg_data.parquet"
...
def _build_train_ns(cfg: Dict[str, Any]) -> SimpleNamespace:
    ...
    return SimpleNamespace(
        ...
        DATA_LOADER_WORKERS=int(ner_cfg.get("data_loader_workers", 0)),  # Default to 0 (single-threaded)
    )
...
def load_configs_from_yaml(path: str | Path) -> Tuple[SimpleNamespace, SimpleNamespace, SimpleNamespace]:
    ...
    cfg = _stage_from_pipeline(cfg, stage_key="ingredient_ner", sub_key="training")
...
def load_inference_configs_from_yaml(path: str | Path) -> Tuple[SimpleNamespace, SimpleNamespace]:
    ...
    cfg = load_full_yaml(path)
    cfg = _stage_from_pipeline(cfg, stage_key="ingredient_ner", sub_key="inference")
