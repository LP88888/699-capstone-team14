...
def docs_from_text_and_labels(df: pd.DataFrame, text_col: str, cuisine_col: str) -> List[Doc]:
    ...
    Handles both string and list-like ingredient columns. Each ingredient in a list
    is kept as a separate item (not concatenated into a long string) for better
    representation of short text spans.
    """
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Creating docs from text and labels"):
        ...
        # Join tokens with comma - keeps individual ingredients distinguishable
        # Using comma separator instead of concatenation preserves ingredient boundaries
...
def docs_from_ingredients_per_item(df: pd.DataFrame, text_col: str, cuisine_col: str) -> List[Doc]:
    """
    Create spaCy Docs treating each individual ingredient as a separate training example.
    
    This is useful when you want the model to learn from short text spans rather than
    long concatenated ingredient lists. Each ingredient gets the cuisine label of its recipe.
    """
    blank = spacy.blank("en")
    out: List[Doc] = []
    ...
    all_labels = sorted(list(unique_cuisines))
    logger = __import__("logging").getLogger(__name__)
    logger.info(f"Found {len(all_labels)} unique cuisine labels (per-ingredient mode)")
    
    # Second pass: Create one doc per ingredient
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Creating docs per ingredient"):
        text_val = row[text_col]
        text_tokens = parse_listlike(text_val)
        ...
        # Create one doc per ingredient
        for ingredient in text_tokens:
...
def build_docs_from_config() -> Tuple[List[Doc], List[Doc], str]:
    ...
    # Read sample to get available columns
    ...
    logger.info(f"[DEBUG] Available columns: {available_cols}")
    
    # Auto-detect text column if not set or not found
    text_col = DATA.TEXT_COL
    if not text_col or text_col not in available_cols:
        text_col_candidates = [
            "ingredients", "ingredient", "ingredients_list", "ingredient_list",
            "NER", "NER_clean", "text", "content"
        ]
        for candidate in text_col_candidates:
            for col in available_cols:
                if col.lower() == candidate.lower():
                    text_col = col
                    logger.info(f"[DEBUG] Auto-detected text column: '{text_col}'")
                    break
            if text_col and text_col in available_cols:
                break
    
    # Auto-detect cuisine column if not set or not found
    cuisine_col = DATA.CUISINE_COL
    if not cuisine_col or cuisine_col not in available_cols:
        cuisine_col_candidates = [
            "cuisine", "cuisines", "category", "type", "label", "class"
        ]
        for candidate in cuisine_col_candidates:
            for col in available_cols:
                if col.lower() == candidate.lower():
                    cuisine_col = col
                    logger.info(f"[DEBUG] Auto-detected cuisine column: '{cuisine_col}'")
                    break
            if cuisine_col and cuisine_col in available_cols:
                break
    
    # Validate columns exist
    if not text_col or text_col not in available_cols:
        raise RuntimeError(
            f"Text column not found. Available columns: {available_cols}\n"
            f"Set text_col in config to one of: {available_cols}"
        )
    if not cuisine_col or cuisine_col not in available_cols:
        raise RuntimeError(
            f"Cuisine column not found. Available columns: {available_cols}\n"
            f"Set cuisine_col in config to one of: {available_cols}"
        )
    ...
    # Create docs
    docs_all = docs_from_text_and_labels(df, text_col, cuisine_col)
...
__all__ = [
    ...
    "docs_from_ingredients_per_item",
